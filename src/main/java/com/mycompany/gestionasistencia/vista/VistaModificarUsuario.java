...
public class VistaModificarUsuario extends javax.swing.JFrame {
    ...
    Usuarios usuarioModificar = null;
    ...
    // botones
    // datos usuario
    JPanel botonDatosUsuario;
    JLabel labelBotonDatosUsuario;
    
    JPanel botonResetearDatos;
    JLabel labelResetearDatos;
    
    // jornada usuario
    JPanel botonJornadaSiguiente;
    JLabel labelJornadaSiguiente;
    
    JPanel botonJornadaAnterior;
    JLabel labelJornadaAnterior;
    
    public VistaModificarUsuario(Usuarios usuario) {
        initComponents();
    ...
    private void crearInputsDatosUsuario (){
        ...
        lblAgregarUsuarios.setText("Modificar datos");
        ...
        lblAgregarUsuarios.setBounds((int) Math.round(panelTabsWidth * 0.01),(int) Math.round(panelTabsHeight * 0.01), anchoBotones * 3, altoBotones);
    ...
    private void crearBotones (){
        // datos usuario
        botonDatosUsuario = new JPanel();
        labelBotonDatosUsuario = new JLabel();
        crearBoton(
                botonDatosUsuario, 
                labelBotonDatosUsuario, 
                "Siguiente", 
                panelDatosUsuario, 
                panelTabs.getWidth() - anchoBotones * 2,
                panelTabs.getHeight() - altoBotones * 3
        );
        
        botonResetearDatos = new JPanel();
        labelResetearDatos = new JLabel();
        ...
        // jornadas
        botonJornadaSiguiente = new JPanel();
        labelJornadaSiguiente = new JLabel();
        
        botonJornadaAnterior = new JPanel();
        labelJornadaAnterior = new JLabel();
        
        crearBoton(
                botonJornadaSiguiente, 
                labelJornadaSiguiente, 
                "Modificar usuario", 
                panelJornadaUsuario, 
                panelTabs.getWidth() - anchoBotones * 2,
                panelTabs.getHeight() - altoBotones * 3
        ...
        crearBoton(
                botonJornadaAnterior, 
                labelJornadaAnterior, 
                "Anterior", 
                panelJornadaUsuario, 
                anchoBotones,
                panelTabs.getHeight() - altoBotones * 3
        ...
        botonResetearDatos.addMouseListener(new MouseAdapter(){
            @Override
    ...
    private void ajustarVentana() {
        addComponentListener(new ComponentAdapter(){
            ...
            public void componentResized(ComponentEvent e){
                ...
                labelBotonDatosUsuario.setFont(new Font("Segoe UI Symbol", 1, fontSize));
                labelBotonDatosUsuario.setBounds(0, 0, botonDatosUsuario.getWidth(), botonDatosUsuario.getHeight());
                
                botonResetearDatos.setBounds(anchoBotones, panelTabsHeight - altoBotones * 3, anchoBotones, altoBotones);
                labelResetearDatos.setFont(new Font("Segoe UI Symbol", 1, fontSize));
                labelResetearDatos.setBounds(0, 0, botonResetearDatos.getWidth(), botonResetearDatos.getHeight());
                
                botonJornadaSiguiente.setBounds(panelTabsWidth - anchoBotones * 2, panelTabsHeight - altoBotones * 3, anchoBotones, altoBotones);
                labelJornadaSiguiente.setFont(new Font("Segoe UI Symbol", 1, fontSize));
                labelJornadaSiguiente.setBounds(0, 0, botonJornadaSiguiente.getWidth(), botonJornadaSiguiente.getHeight());
                
                botonJornadaAnterior.setBounds(anchoBotones, panelTabsHeight - altoBotones * 3, anchoBotones, altoBotones);
                labelJornadaAnterior.setFont(new Font("Segoe UI Symbol", 1, fontSize));
                labelJornadaAnterior.setBounds(0, 0, botonJornadaAnterior.getWidth(), botonJornadaAnterior.getHeight());
            }
        });
    }
    
    private void crearBoton (JPanel btn, JLabel lbl, String txt, JPanel panel,int posX, int posY){
        int fontSize = (int) Math.floor(Math.min(width, height) * 0.02);
        panel.add(btn);
        btn.add(lbl);
        
        btn.setBackground(new Color(102,51,255));
        btn.setLayout(null);
        btn.setBounds(posX, posY, anchoBotones, altoBotones);
        
        lbl.setText(txt);
        lbl.setForeground(Color.white);
        lbl.setFont(new Font("Segoe UI Symbol", 1, fontSize));
        lbl.setHorizontalAlignment(SwingConstants.CENTER);
        lbl.setBounds(0, 0, btn.getWidth(), btn.getHeight());
    }
    ...
    private void crearUsuario() throws Exception{
        if (verificarDatosNuevoUsuario() && verificarJornadaNuevoUsuario()){
            ...
            usuarioModificar.setRut(rut);
            usuarioModificar.setNombre(nombre);
            usuarioModificar.setTelefono(telefono);
            usuarioModificar.setCorreo(correo);
            usuarioModificar.setSueldo(Integer.parseInt(sueldo));
            usuarioModificar.setContra(contra);
            usuarioModificar.setRol(rol);
            usuarioModificar.setGerencia(gerencia);
            usuarioModificar.setDepartamento(departamento);
            usuarioModificar.setCargo(cargo);
            usuarioModificar.setFechaCreacion(fechaActual);
            
            try {
                controlador.modificarUsuario(usuarioModificar);
                JornadasDeTrabajo jornadaUsuario = usuarioModificar.getContrato().getJornadasDeTrabajo();
                ...
                jornadaUsuario.setHoraEntrada(timeEntrada);
                jornadaUsuario.setHoraSalida(timeSalida);
                
                controlador.modificarJornadaUsuario(jornadaUsuario);
                
                List<JornadaDias> listaDiasJornada = jornadaUsuario.getJornadaDias();
                // eliminar jornada anterior
                for (JornadaDias jornadaDia : listaDiasJornada){
                    try{
                        controlador.borrarJornadaDias(jornadaDia.getId());
                    }catch (NonexistentEntityException e){
                        JOptionPane.showMessageDialog(null, "Error al borrar los dias de la jornada", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
                ...
                for (Object[] objeto : listaBotonesDias){
                    ...
                    for (DiasDeTrabajo diaDeTrabajo : listaDias){
                        if (diaDeTrabajo.getDia().equals(dia) && botonCasteado.isSelected()){
                            ...
                            nuevaJornadaDias.setJornadaDeTrabajo(jornadaUsuario);
    ...
    private Boolean verificarDatosNuevoUsuario(){
        ...
        if (!rut.matches(rutRegex)){
            ...
            if (usuarioRut != null && !rut.equals(usuarioModificar.getRut())){
        ...
        if (!telefono.matches(telefonoRegex)){
            ...
            if (usuarioTelefono != null && !telefono.equals(usuarioModificar.getTelefono())){
        ...
        if (!correo.matches(correoRegex)){
            ...
            if (usuarioCorreo != null && !correo.equals(usuarioModificar.getCorreo())){
    ...
    private void resetValues () {
        fieldRut.setText(usuarioModificar.getRut());
        fieldNombre.setText(usuarioModificar.getNombre());
        fieldSueldo.setText(String.valueOf(usuarioModificar.getSueldo()));
        fieldTelefono.setText(usuarioModificar.getTelefono());
        fieldCorreo.setText(usuarioModificar.getCorreo());
        fieldContra.setText(usuarioModificar.getContra());
        
        for (int i = 0; i < comboGerencias.getItemCount(); i++) {
            Gerencias gerencia = (Gerencias) comboGerencias.getItemAt(i);
            if (gerencia.getGerencia().equals(usuarioModificar.getGerencia().getGerencia())){
                comboGerencias.setSelectedIndex(i);
            }
        }
        
        for (int i = 0; i < comboDepartamentos.getItemCount(); i++) {
            Departamentos departamento = (Departamentos) comboDepartamentos.getItemAt(i);
            if (departamento.getDepartamento().equals(usuarioModificar.getDepartamento().getDepartamento())){
                comboDepartamentos.setSelectedIndex(i);
            }
        }
        
        for (int i = 0; i < comboCargos.getItemCount(); i++) {
            Cargos cargo = (Cargos) comboCargos.getItemAt(i);
            if (cargo.getCargo().equals(usuarioModificar.getCargo().getCargo())){
                comboCargos.setSelectedIndex(i);
            }
        }
        
        
        Time horaEnt = usuarioModificar.getContrato().getJornadasDeTrabajo().getHoraEntrada();
        Time horaSal = usuarioModificar.getContrato().getJornadasDeTrabajo().getHoraSalida();
        LocalTime localTimeEntrada = horaEnt.toLocalTime();
        LocalTime localTimeSalida = horaSal.toLocalTime();
        
        int horasEntrada = localTimeEntrada.getHour();
        int minutosEntrada = localTimeEntrada.getMinute();
        
        int horasSalida = localTimeSalida.getHour();
        int minutosSalida = localTimeSalida.getMinute();
        
        pEntradaHoras.setValue(horasEntrada);
        pEntradaMinutos.setValue(minutosEntrada);

        pSalidaHoras.setValue(horasSalida);
        pSalidaMinutos.setValue(minutosSalida);
        
        List<JornadaDias> listaDias = usuarioModificar.getContrato().getJornadasDeTrabajo().getJornadaDias();
        
        for (Object[] objeto : listaBotonesDias){
            JRadioButton btnCasteado = (JRadioButton) objeto[0];
            String diaBtn = (String) objeto[2];
            for (JornadaDias dia : listaDias){
                if (dia.getDiasDeTrabajo().getDia().equals(diaBtn)){
                    btnCasteado.setSelected(true);
                }
            }
        }
    }
